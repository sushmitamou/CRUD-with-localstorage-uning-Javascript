...
function addDataInLocalStorage(newTask){
    ...
    var allMembers = storeData();

    newTask.id = guid();
    allMembers.push(newTask);
    localStorage.setItem('tasks2', JSON.stringify(allMembers));
}
...
function cloneInfo() {
    ...
    icon.className = 'fa fa-briefcase';
    occupation.appendChild(icon);
    occupation.appendChild(document.createTextNode(document.getElementById('designation').value));
